...
// 獲取用戶的文章列表
export async function getUserPosts(userId, { page = 1, limit = 10, status = null } = {}) {
  const offset = (page - 1) * limit;
...
// 獲取用戶的文章總數
export async function getUserPostsCount(userId, status = null) {
  let sql = 'SELECT COUNT(*) as total FROM posts WHERE user_id = ?';
  let args = [userId];
